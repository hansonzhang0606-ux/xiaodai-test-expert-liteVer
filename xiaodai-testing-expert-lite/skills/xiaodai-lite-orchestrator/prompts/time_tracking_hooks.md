...
4. **脚本级强制（v6.3，不依赖提示词自觉）**：`secure_record_time_saved.py` 在写盘前会自行调用检测脚本，
   核对编号更小的追踪环节是否已记录。若存在缺失，**拒绝落盘**并返回退出码 `2`，stderr 输出 JSON：

   ```json
   {"status":"blocked","step_code":"04","missing":["01"],"message":"环节 04 的记录被拦截..."}
   ```

   处理约定：
   - 收到退出码 `2` → **必须**先按编号升序补录 `missing` 中列出的环节（人员可回复「跳过」）；
   - 仅当人员对每个缺失环节都明确了「补录值」或「跳过」后，才对当前环节加
     `--force-with-missing --missing-reason "人员跳过环节01"` 重试写入；
   - **禁止**在未向人员提示的情况下直接使用 `--force-with-missing`（否则脚本强制形同虚设）；
   - 跳过原因会写入该条记录的 `remark`，便于事后追溯。
5. 全部缺失环节处理完毕后，才进入本步骤的正常时间反馈流程（见下方固定顺序）。
6. 检测脚本自身调用失败（非 0 退出码）→ 说明原因并跳过补录，直接进入正常流程，不阻塞测试工作。
...
## 全环节完成核验（v6.1；v6.3 起覆盖「所有已执行环节」）

当本次会话针对某用户故事的全部环节执行完毕，或会话收尾时，
**必须对每个已执行用户故事做一次端到端同步核验**，确保「本地已记录」与「MySQL 已同步」一致。

**核验范围 = 本次会话实际执行过的追踪环节**，不再局限于主链路 01/04/06：
- 主链路 01（需求整理）/ 04（测试点）/ 06（用例）为必检项；
- 若本次会话执行过按需环节 **02（需求评审）、07（知识库入库）**，**必须一并纳入核验**；
- `verify_story_sync.py` 的 `--steps` 留空即「本地已记录的全部环节」，可直接使用，避免漏检。
...
1. 本地记录核查（缺记录先补录）：
   ...
   python time-tracking-skill/scripts/check_missing_time_records.py \
     ...
     --user-story "{用户故事}" \
     --steps "{已执行环节，逗号分隔；如 01,04,06 或 01,02,04,06,07}"
   ```
   `missing` 非空 → 按编号升序逐个补录（走正常时间反馈流程），直到已执行环节齐全。
...
2. MySQL 同步核查（未同步立即补同步）：
   ...
   python time-tracking-skill/scripts/verify_story_sync.py \
     ...
     --user-story "{用户故事}"
   ```
   （不传 `--steps` 即核验本地已记录的全部环节，自动覆盖按需环节，避免漏检）
